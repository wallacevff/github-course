...
void insere_ordenada(Lista **l, int c, char * elem)
{
	No *novo,*aux;
...
void enumera(Lista **l)
{
	...
	while(aux!=NULL)
	{
		printf("Chave: %d",aux->chave);printf("  Nome: %s\n",aux->nome);
		aux=aux->prox;
	}
...
void escreve_arquivo(){
	...
	while(opcao != 4){
		...
		switch(opcao){
			case 1:
				printf("\nDigite um nome: "); nome = getNome();
				if(busca(&l, nome) == 1) { printf("\nNome ja cadastrado\n"); } else{ insere_ordenada(&l, i++,nome); system("cls");}
